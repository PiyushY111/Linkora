...
- The memory budget ([docs/redis-keys.md](../redis-keys.md)) shows the steady state at about 46 MiB of 256 MB at free-tier volumes.
...
- With `noeviction`, a full instance **rejects writes** instead of evicting. Redirects keep working, because cache writes are best-effort and a cache read miss falls back to MongoDB. But click events, logins and rate limits would fail. The TTLs and caps are what keep this from happening, and the budget shows the margin.
...
- Consumer downtime is bounded by `CLICK_STREAM_MAXLEN`. Beyond it, the oldest unprocessed clicks are trimmed and lost, and that's logged.
