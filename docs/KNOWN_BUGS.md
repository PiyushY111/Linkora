...
| 7 | The global in-memory limiter in `app.js` (`RATE_LIMIT_MAX_REQUESTS`, default 100 per 15 min per IP in production) also applies to redirects, so visitors behind a shared IP (office, carrier NAT) get 429s on a popular link. Redirects need their own, much higher limit. | `backend/src/app.js` | Step 8 |
