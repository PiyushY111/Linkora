...
const qrFallbackSrc = (link) =>
  link.qrCode || `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(link.shortUrl)}`;

export default function QrAssetCard({ link, onCustomize, onDownload }) {
  return (
    <div className="panel p-4 flex items-center justify-between gap-4">
      <div className="flex items-center gap-4 min-w-0">
        <div onClick={onCustomize} className="relative group cursor-pointer shrink-0" title="Click to customize QR code">
          {link.qrConfig ? (
            <div className="h-20 w-20 rounded-xl bg-ink-950 p-1 ring-1 ring-ink-600 transition-transform group-hover:scale-105 flex items-center justify-center overflow-hidden">
              <QRCodeViewer data={link.shortUrl} config={link.qrConfig} size={72} showFrame={false} />
            </div>
          ) : (
            <img
              src={qrFallbackSrc(link)}
              alt="QR Code"
        ...
        <div className="space-y-1.5 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-paper-300">Dynamic QR Asset</h4>
            <span className="badge-accent text-[10px]">Editable Target</span>
            {link.qrConfig && <span className="badge-neutral text-[10px]">Customized</span>}
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            <button type="button" onClick={onCustomize} className="btn-primary btn-sm">
              <Sparkles size={12} />
